...
| ~~A5~~ | ~~Hunt for an immune ground-truth cohort~~ | **DELIVERED 2026-09-15 by you, and it produced the project's first external validation** | You supplied GBMdeconvoluteR (Ajaib et al., Neuro-Oncology 2023;25(7):1236–1248): imaging mass cytometry, 33 antibodies, ten IDHwt GBM with matched bulk. **The repo contains marker sets, not the IMC counts** — that cohort is the authors' own and is not public through this route — so the anchor used is their published verdict rather than a re-analysis. A prediction was pre-specified, committed, and then **failed**: ACS ranked GBmap-derived markers (0.6667) above the GBM-specific markers IMC ranks first (0.5128), paired p = 0.0090. Follow-up showed ACS moving 0.3846 → 0.6923 on nothing but marker-set size. `docs/EXTERNAL_VALIDATION.md`, correction **C10**. Superseded reasoning follows. |
| ~~A5-old~~ | ~~superseded reasoning~~ | ~~open, highest external value~~ | Still the highest scientific value externally. Every method over-calls T cells 4.6–7.9× in high-purity tumour and **ACS cannot see it** — `T_cell` carries no constraint. Real measured immune content either confirms that on tissue or shows it is a pseudobulk artefact. |
...
| **A6** | Wire **CDSeq** on the anatomic arm | **UNBLOCKED 2026-09-15 by you; feasibility MEASURED; mine to do** | The portal publishes per-sample RSEM `genes.results` carrying **`expected_count`**, which GEO does not mirror. All 270 fetched and gated. CDSeq 1.0.9 installed and **run end-to-end on real Ivy GAP counts**: 300 genes x 12 samples, T=4, 150 iterations, **11.7 s**, `estProp` columns summing to 1. The cost driver is reads, not genes — CDSeq's Gibbs sampler walks every read, so `dilution_factor=1` on the same subset was 53M reads x 150 iterations = 8e9 draws and had not finished in 4 minutes, while `dilution_factor=100` finished in 11.7 s. Extrapolated for the real run (2,000 genes x 122 anatomic samples, T=8, 1,000 iterations, dilution 100): **~30 min single-core**, less with `block_number`/`cpu_number`. This is the one instrument that tests **C4** without a reference. | CDSeq is a multinomial **read-count** model. The Ivy GAP bulk is FPKM rescaled to 1e6 — non-integer, columns summing to exactly 1000000.0 — and the published archive contains **four files**: `fpkm_table.csv`, `columns-samples.csv`, `rows-genes.csv`, `README.txt`. **No counts exist to give it.** See A22. A partial path needs nothing from you: CDSeq *can* run on the pseudobulk arm, whose mixtures rebuild from `raw/X` counts, and that still answers "is it the methods or the reference?" from the reference-free side. I will do that. |
| ~~A22~~ | ~~Ask the Allen Institute for Ivy GAP read counts~~ | **RESOLVED 2026-09-15 — you found them yourself; no email needed** | The counts were public all along, on the portal's download page rather than in GEO. See **C9**, which is now a retraction. Superseded reasoning follows. |
| ~~A22-old~~ | ~~superseded reasoning~~ | ~~closed on GEO's wording~~ | You supplied **GSE107559**, which answers it definitively and better than a reply would have. GEO states *"Raw data not provided for this record"* and *"The raw RNA-Seq and SNP array data will be submitted to dbGaP."* Its FPKM table is **bit-identical** to the archive we already load (25,874 lines, same SHA-256), so the two public distributions are the same data and **neither has counts**. Counts sit behind dbGaP controlled access (PRJNA420740), which needs an institutional signing official. Recorded as a **limitation** in **C9**, not as pending work. |
...
| 12 / A5 | immune ground truth | **DELIVERED** — first external validation; pre-specified prediction FAILED; see **C10** | — |
| **A24** | a SECOND external ground truth, ideally with the per-sample counts | **NEW, OPEN** — C10 rests on one study's published summary, nine tumours | you |
