#!/usr/bin/env python3
"""
remeasure_dwls.py — run the GENUINE DWLS R package on the anatomic cohort, unbounded.
...
def main() -> int:
    ...
    ap.add_argument("--out", default="results/dwls_remeasured.json")
    args = ap.parse_args()
    ...
    print(f"\nrunning the genuine DWLS R package with a {args.budget}s budget "
          f"(the pipeline's is {r_bridge.timeout_for('dwls')}s)...")
    t0 = time.perf_counter()
    failed = None
    est = None
    try:
        est = r_bridge.run_r_method("dwls", data, timeout=args.budget)
    ...
    report = {
        "what_this_is": ("A single-method re-measurement of the GENUINE DWLS R package on "
    ...
    if est is not None:
        ...
        dwls = next(m for m in build_methods(prefer_r=False) if m.name == "dwls")
        import numpy as np
        ...
        res = acs_score(frame, meta.loc[anat], method="dwls_genuine",
        ...
        print(f"\n  GENUINE DWLS: ACS {s['acs']:.4f} "
              f"[{s['ci_low']:.4f}, {s['ci_high']:.4f}] "
              f"on {s['n_constraint_tumor_pairs']} pairs, null_p {s['null_p']:.4g}")
        print(f"  reimplementation in the confirmatory run: ACS 0.7385")
    else:
        print(f"\n  FAILED: {failed}")
